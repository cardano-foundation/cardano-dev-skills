...
- **PR policy gate** (`.github/workflows/pr-policy.yml`) — on PRs touching `skills/`, `registry/`, or `docs/sources/`: mechanical checks (`scripts/check-pr-policy.py`, hard-fail — live source vetting, brand-named-skill detection, self-containment; a registry entry can waive the recency/activity vetting rules via a declared `vetting_exception` reason, surfaced as a warning — same reviewed-exception pattern as `ALLOWED_TOOLS_EXCEPTIONS` in `validate.py`) plus an advisory AI scope review (a single non-agentic Gemini call posting a sticky verdict comment; skips cleanly when no `GEMINI_API_KEY` secret is configured). Humans still merge.
