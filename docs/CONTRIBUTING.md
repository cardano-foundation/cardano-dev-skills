...
- **Plugin meta-skills are the one recorded exception.** `cardano-context` and `give-feedback` configure or feed back into the plugin itself rather than teach a Cardano workflow; each is justified in DESIGN.md (Decisions 12 and 14). A new one needs its own decision there.
