...
- **A skill may teach a hosted service only as an optional transport, never as a dependency.** The conditions (registered source with the serving module mirrored, delivers without the service, no tool pre-approval, one fixed endpoint, trust boundary stated in the skill's own text, self-hosting as the production path) are DESIGN.md Decision 15; `analyze-chain-data` is the model.
...
- [ ] No external service dependencies — the skill delivers its output with `Read` / `Grep` / `Glob` alone; an optional hosted transport is allowed only under DESIGN.md Decision 15
